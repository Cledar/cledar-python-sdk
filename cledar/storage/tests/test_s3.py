...
def test_upload_file_filesystem_with_bucket_key_should_use_s3(
    ...
    object_storage_service.s3_client.put = MagicMock()

    object_storage_service.upload_file(file_path=file_path, bucket=bucket_name, key=key)

    object_storage_service.s3_client.put.assert_called_once_with(
...
def test_read_file_filesystem_with_bucket_key_should_use_s3(
    ...
    object_storage_service.s3_client.open = MagicMock(
    ...
    object_storage_service.s3_client.open.assert_called_once_with(
...
def test_has_bucket_no_throw_not_exists(
    ...
    bucket_name = fake.name()
    object_storage_service.s3_client.ls.side_effect = OSError("Connection failed")
    result = object_storage_service.has_bucket(bucket=bucket_name)
...
def test_upload_buffer_exception(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.open.assert_called_once()
...
def test_upload_file_exception(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.put.side_effect = OSError("Network error")
...
def test_read_file_exception(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.open.side_effect = ReadFileError
...
def test_list_objects_recursive_s3(
    ...
    object_storage_service.s3_client.find.return_value = mock_objects
    ...
    object_storage_service.s3_client.find.assert_called_once()
...
def test_list_objects_non_recursive_s3(
    ...
    object_storage_service.s3_client.ls.return_value = mock_objects
    ...
    assert f"{prefix}/file1.txt" in result
    assert f"{prefix}/file2.txt" in result
    object_storage_service.s3_client.ls.assert_called_once()


def test_list_objects_exception(object_storage_service: ObjectStorageService) -> None:
    bucket = fake.name()
    object_storage_service.s3_client.find.side_effect = OSError("List failed")
...
def test_delete_file_s3(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.rm.assert_called_once_with(f"s3://{bucket}/{key}")


def test_delete_file_exception(object_storage_service: ObjectStorageService) -> None:
    bucket = fake.name()
    key = "test/file.txt"
    object_storage_service.s3_client.rm.side_effect = OSError("Delete failed")
...
def test_file_exists_true_s3(object_storage_service: ObjectStorageService) -> None:
    bucket = fake.name()
    key = "test/file.txt"
    object_storage_service.s3_client.exists.return_value = True

    result = object_storage_service.file_exists(bucket=bucket, key=key)

    assert result is True
    object_storage_service.s3_client.exists.assert_called_once_with(f"s3://{bucket}/{key}")


def test_file_exists_false_s3(object_storage_service: ObjectStorageService) -> None:
    bucket = fake.name()
    key = "test/file.txt"
    object_storage_service.s3_client.exists.return_value = False
...
def test_file_exists_exception(object_storage_service: ObjectStorageService) -> None:
    bucket = fake.name()
    key = "test/file.txt"
    object_storage_service.s3_client.exists.side_effect = OSError("Check failed")
...
def test_download_file_s3(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.get.assert_called_once_with(
...
def test_download_file_retry_success(
    ...
    object_storage_service.s3_client.get.side_effect = [
    ...
    assert object_storage_service.s3_client.get.call_count == 3
...
def test_download_file_exception_after_retries(
    ...
    key = "test/file.txt"
    dest_path = "/tmp/downloaded_file.txt"
    object_storage_service.s3_client.get.side_effect = OSError("Network error")
...
def test_get_file_size_s3(object_storage_service: ObjectStorageService) -> None:
    ...
    key = "test/file.txt"
    expected_size = 1024
    object_storage_service.s3_client.info.return_value = {"size": expected_size}

    result = object_storage_service.get_file_size(bucket=bucket, key=key)

    assert result == expected_size
    object_storage_service.s3_client.info.assert_called_once_with(f"s3://{bucket}/{key}")


def test_get_file_size_exception(object_storage_service: ObjectStorageService) -> None:
    bucket = fake.name()
    key = "test/file.txt"
    object_storage_service.s3_client.info.side_effect = OSError("Info failed")
...
def test_get_file_info_s3(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.info.return_value = expected_info
...
def test_copy_file_s3_to_s3(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.copy.assert_called_once_with(
...
def test_copy_file_exception(object_storage_service: ObjectStorageService) -> None:
    ...
    dest_bucket = fake.name()
    dest_key = "test/destination.txt"
    object_storage_service.s3_client.copy.side_effect = OSError("Copy failed")
...
def test_move_file_s3_to_s3(object_storage_service: ObjectStorageService) -> None:
    ...
    object_storage_service.s3_client.move.assert_called_once_with(
        f"s3://{source_bucket}/{source_key}", f"s3://{dest_bucket}/{dest_key}"
    )
...
def test_move_file_exception(object_storage_service: ObjectStorageService) -> None:
    ...
    dest_bucket = fake.name()
    dest_key = "test/destination.txt"
    object_storage_service.s3_client.move.side_effect = OSError("Move failed")
